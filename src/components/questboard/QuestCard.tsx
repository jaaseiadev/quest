...
export function QuestCard({
  ...
  return (
    <Card variant="interactive" className="flex h-full flex-col gap-6">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {applicationStatus ? <StatusChip status={applicationStatus}>Applied</StatusChip> : null}
      </div>
...
function QuestMeta({
  ...
  return (
    <div className="border border-border bg-surface-container px-3 py-3">
      <dt className="flex items-center gap-2 text-label-caps text-muted-foreground">
        <Icon className="h-4 w-4 text-primary" aria-hidden={true} />
        {label}
      </dt>
      <dd className={`mt-2 font-display text-sm font-semibold ${toneClass}`}>{value}</dd>
    </div>
  );
}
